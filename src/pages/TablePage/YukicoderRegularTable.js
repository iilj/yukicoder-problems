      ...
      >
        <TableHeaderColumn
          ...
          columnClassName={(_, contest) => {
            ...
            if (showContestResult && ls.every(stat => stat === 3))
              return "table-problem table-problem-solved-before-contest";
            else if (showContestResult && ls.every(stat => stat >= 2))
              return "table-problem table-problem-solved-intime";
            else if (ls.every(stat => stat >= 1))
              return "table-problem table-problem-solved";
            else
              return "table-problem";
          }}
          dataFormat={(_, contest) => (
            <ContestLink contestId={contest.Id} contestName={contest.Name.replace(/^yukicoder contest /, '')} />
          )}
        >
          Contest
        </TableHeaderColumn>
